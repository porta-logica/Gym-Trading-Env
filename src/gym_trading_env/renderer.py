...
class Renderer:
    def __init__(self, render_logs_dir):
        ...
        self.lines = []
        mr = 100 * (df['close'].iloc[-1] / df['close'].iloc[0] - 1)
        pr = 100 * (df['portfolio_valuation'].iloc[-1] / df['portfolio_valuation'].iloc[0] - 1)
        self.metrics = [
            {
                "name": "Market Return",
                "function": lambda df: f"{mr:0.2f}%",
            },
            {
                "name": "Portfolio Return",
                "function": lambda df: f"{pr:0.2f}%"
            },
        ]
