...
class TradingEnv(gym.Env):
    ...
    :param initial_position: You can specify the initial position of the environment or set it to 'random'.
     It must be contained in the list parameter 'positions'.
    ...
    :param name: The name of the environment (e.g. 'BTC/USDT')
    ...
    def _take_action_order_limit(self):
        if len(self._limit_orders) > 0:
            ...
            for position, params in self._limit_orders.items():
                if position != self._position and ticker["high"] >= params['limit'] >= ticker["low"]:
...
class MultiDatasetTradingEnv(TradingEnv):
    ...
    :param dataset_dir: A `glob path <https://docs.python.org/3.6/library/glob.html>`_
     that needs to match your datasets.
      All of your datasets needs to match the dataset requirements (see docs from TradingEnv).
       If it is not the case, you can use the ``preprocess`` param to make your datasets match the requirements.
    :type dataset_dir: str

    :param preprocess: This function takes a pandas.DataFrame and returns a pandas.DataFrame.
     This function is applied to each dataset before being used in the environment.
    ...
    :param episodes_between_dataset_switch: Number of times a dataset is used to create an episode,
     before moving on to another dataset. It can be useful for performances when `max_episode_duration` is low.
